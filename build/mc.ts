...
const CRAFATAR = "https://avatars.cloudhaven.gg"; // todo: change back to crafatar
...
export async function donwloadPlayerAssets(player: keyof typeof PLAYERS) {
  ...
  // skin
  const skin = await downloadImage(`${VISAGE}/processedskin/${uuid}`);
  ...
  // cape
  if (cape) {
    const capeUrl = await fetch(`${CAPES}/load/${uuid}/${cape}`)
      .then((res) => res.json())
